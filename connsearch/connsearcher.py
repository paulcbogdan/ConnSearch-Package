...
from connsearch.corrsim import corrsim_all_components
from connsearch.utils import print_list_stats, get_t_graph, get_groups
...
class ConnSearcher:
    def __init__(self, X: np.ndarray,
                 ...
                 components: Union[list, np.ndarray],
                 dir_out: Union[None, str],
                 n_splits: Union[None, int] = None,
                 n_repeats: Union[None, int] = None,
                 clf=None,
                 wu_analysis: bool = False) -> None:
        '''
        ...
        n_splits, n_repeats, and clf are only used for group-level analyses.
        ...
        assert len(self.X.shape) == 5, \
            'X must be 5D: (subject, session, example, ROI0, ROI1)'
        self.Y = Y
    ...
    def do_group_level_analysis(self,
                                acc_thresh: Union[None, float] = None,
                                acc2p: Union[None, Callable] = None,
                                alpha: float = 0.05,
                                verbose: bool = True,
                                seed: int = 0) -> list:
        '''
        ...
        To determine what the group-level accuracy threshold is needed for
            ...
            permute-testing (the Permutation_Manager class).
        ...
        :return: list of mean accuracies yielded by each component classifier
        '''
        np.random.seed(seed)
        ...
        for i, X_component in iterator:
            ...
            acc = cross_val_score(clone(self.clf), X_component, Y_reshaped,
                                  cv=cv, groups=groups)
            acc = np.mean(acc)
            acc = np.float64(acc)  # Prevent hinting warning below
            comp_matrix = \
    ...
    def do_subject_specific_analysis(self, alpha: float = .05,
                                     FWE: bool = True,
                                     subtract_mean: bool = False,
                                     multiple_tests_method: Union[None, str] = None) -> None:
        '''
        ...
        The bulk of the code is done using the methods in
            corrsim.py. Said code leverages np arrays and
    ...
    def save_component_ConnSearch(self, component_i: int,
                                  score: float,
                                  comp_matrix=None,
        ...
        :param dir_out: str, directory to save component .pkl. If None, uses
                        self.dir_out
        ...
        # The result dict will be saved and can be loaded for making tables and plotting
        result = {
            ...
            'component': component,  # indices (ROI_i) of ROIs in component. component[0] is the Core ROI's index
    ...
    def save_component_Wu(self, component_i: int,
                          score: float,
                          comp_matrix=None,
                          dir_out: Union[None, str] = None,
                          code_xtra: str = '') -> None:
        ...
        result = {'score': score,
                   'coords_component': coords_component,
                   'adj': t_component,
                   'coords_all': self.coords,
                   'component': component,
                   'edges': edges,
